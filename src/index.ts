...
import cfu from './update';
import { getLatest } from './update';

const pkg:any = JSON.parse(readFileSync(path.join(__dirname, '../package.json'), 'utf-8'));
let beta:boolean = false;
if (pkg.status === "Alpha" || pkg.status === "Beta" || pkg.status === "RC") {
  beta = true;
}
const version:string = pkg.version.toString();
...
async function main() {
  ...
  program.command('webUI')
      ...
      .action(async function(options) {
        ...
        webUI(port, folder, beta, version);
  })
